...
var d = class extends y {
  constructor(e, t, o = new n.Doc(), {
    ...
    this.initSyncListeners = () => {
      this.socket.on("sync-step-1", (e, t) => {
        t(n.encodeStateAsUpdate(this.doc, new Uint8Array(e))), this.synced = !0;
      }), this.socket.on("sync-update", this.onSocketSyncUpdate);
    };
    this.initAwarenessListeners = () => {
      this.socket.on("awareness-update", (e) => {
        s.applyAwarenessUpdate(this.awareness, new Uint8Array(e), this);
      });
    };
    ...
    this.onSocketConnection = (e = -1) => {
      this.emit("status", [{ status: "connected" }]), this.socket.emit("sync-step-1", n.encodeStateVector(this.doc), (t) => {
        n.applyUpdate(this.doc, new Uint8Array(t), this);
      }), this.awareness.getLocalState() !== null && this.socket.emit("awareness-update", s.encodeAwarenessUpdate(this.awareness, [this.doc.clientID])), this.disableBc || this.connectBc(), e > 0 && (this.resyncInterval = setInterval(() => {
        this.socket.disconnected || this.socket.emit("sync-step-1", n.encodeStateVector(this.doc), (t) => {
          n.applyUpdate(this.doc, new Uint8Array(t), this);
        });
      }, e));
    };
    this.onSocketDisconnection = (e) => {
      this.emit("connection-close", [e, this]), this.synced = !1, s.removeAwarenessStates(this.awareness, Array.from(this.awareness.getStates().keys()).filter((t) => t !== this.doc.clientID), this), this.emit("status", [{ status: "disconnected" }]);
    };
    ...
    this.onSocketSyncUpdate = (e) => {
      n.applyUpdate(this.doc, new Uint8Array(e), this);
    };
    this.awarenessUpdate = ({ added: e, updated: t, removed: o }, c) => {
      let i = e.concat(t).concat(o);
      this.socket.emit("awareness-update", s.encodeAwarenessUpdate(this.awareness, i)), this.bcconnected && a.publish(this._broadcastChannel, {
        type: "awareness-update",
        data: s.encodeAwarenessUpdate(this.awareness, i)
      }, this);
    };
    this.beforeUnloadHandler = () => {
      s.removeAwarenessStates(this.awareness, [this.doc.clientID], "window unload");
    };
    this.connectBc = () => {
      this.bcconnected || (a.subscribe(this._broadcastChannel, this.onBroadcastChannelMessage), this.bcconnected = !0), a.publish(this._broadcastChannel, { type: "sync-step-1", data: n.encodeStateVector(this.doc) }, this), a.publish(this._broadcastChannel, { type: "query-awareness", data: null }, this), a.publish(this._broadcastChannel, { type: "awareness-update", data: s.encodeAwarenessUpdate(this.awareness, [this.doc.clientID]) }, this);
    };
    this.disconnectBc = () => {
      a.publish(this._broadcastChannel, {
        type: "awareness-update",
        data: s.encodeAwarenessUpdate(this.awareness, [this.doc.clientID], /* @__PURE__ */ new Map())
      ...
      if (t !== this && e.type.length > 0)
        switch (e.type) {
          case "sync-step-1":
            a.publish(this._broadcastChannel, {
              type: "sync-step-2",
              data: n.encodeStateAsUpdate(this.doc, e.data)
            }, this);
            break;
          case "sync-step-2":
            n.applyUpdate(this.doc, new Uint8Array(e.data), this);
            break;
          case "sync-update":
            n.applyUpdate(this.doc, new Uint8Array(e.data), this);
            break;
          case "query-awareness":
            a.publish(this._broadcastChannel, {
              type: "awareness-update",
              data: s.encodeAwarenessUpdate(this.awareness, Array.from(this.awareness.getStates().keys()))
            }, this);
            break;
          case "awareness-update":
            s.applyAwarenessUpdate(this.awareness, new Uint8Array(e.data), this);
            break;
          default:
            break;
        }
  ...
  connect() {
    this.socket.connected || (this.emit("status", [{ status: "connecting" }]), this.socket.connect(), this.synced = !1);
  }
  disconnect() {
